...
def add_days_rest(df):
    """Compute days_rest: number of calendar days between fixtures for each team.

    Uses date-only comparison (ignores kick-off times), so a team playing on
    Monday and Friday has 4 days rest regardless of match times.
    Default value is 10 when no prior fixture exists in the dataset.
    """
    df = df.copy()
    home_days_rest = []
    away_days_rest = []

    # Track each team's last match date (date-only, no time component)
    team_last_date = {}

    for _, row in df.iterrows():
        home = row["Home_Team"]
        away = row["Away_Team"]
        match_date = row["Date"].normalize()  # strip time, keep date only

        if home in team_last_date:
            home_days_rest.append((match_date - team_last_date[home]).days)
        else:
            home_days_rest.append(10)

        if away in team_last_date:
            away_days_rest.append((match_date - team_last_date[away]).days)
        else:
            away_days_rest.append(10)

        team_last_date[home] = match_date
        team_last_date[away] = match_date

    df["Home_DaysRest"] = home_days_rest
    df["Away_DaysRest"] = away_days_rest
    return df
...
def add_congestion_features(df):
    ...
    - MatchesLast14Days: number of matches in the last 14 days (high = congested)
    - AvgRest3: average rest days across last 3 matches (low = congested)
    """
    df = df.copy()
    home_m14 = []
    away_m14 = []
    ...
    # Track each team's recent match dates
    team_dates = {}  # team -> list of dates (ordered)
    ...
    for _, row in df.iterrows():
        ...
        for team, m14_list, avgrest_list in [(home, home_m14, home_avgrest3),
                                              (away, away_m14, away_avgrest3)]:
            if team not in team_dates:
                team_dates[team] = []

            past_dates = team_dates[team]

            # Matches in last 14 days
            recent = [d for d in past_dates if (date - d).days <= 14]
            ...
            if len(past_dates) >= 3:
                ...
                avgrest_list.append(np.mean(gaps))
            elif len(past_dates) >= 1:
                avgrest_list.append((date - past_dates[-1]).days)
            else:
...
def add_discipline_features(df, window=5):
    """
    Compute rolling discipline/card features from HY, AY, HR, AR, HF, AF columns.
    ...
    # Build long format
    home = df[["Date", "Home_Team", "HY", "HR", "HF"]].copy()
    home.columns = ["Date", "Team", "YellowCards", "RedCards", "Fouls"]
    away = df[["Date", "Away_Team", "AY", "AR", "AF"]].copy()
    away.columns = ["Date", "Team", "YellowCards", "RedCards", "Fouls"]
...
def add_advanced_features(df):
    ...
    # Consensus features: average across all 4 Poisson/Expected_TG variants to reduce noise
    df["Poisson_Consensus"] = df[["Poisson_xG", "Poisson_Goals", "Poisson_DC", "Poisson_Shots"]].mean(axis=1)
...
def run_pipeline(verbose=True):
    ...
    if verbose:
        print("Computing days_rest feature...")
    df = add_days_rest(df)
    ...
    # Add days_rest features to the feature list
    days_rest_features = ["Home_DaysRest", "Away_DaysRest"]
    all_features_with_days_rest = list(ALL_FEATURES) + days_rest_features

    # Verify all features exist
    missing = [f for f in all_features_with_days_rest if f not in df.columns]
    if missing:
        print(f"WARNING: Missing features: {missing}")
        features = [f for f in all_features_with_days_rest if f in df.columns]
    else:
        features = all_features_with_days_rest
